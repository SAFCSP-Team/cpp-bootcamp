...
[Image of memory address]

## Concept
A `pointer` is a reference variable that stores a memory cell address.

[Image of a pointer referencing a memory address and a value inside that box/memory address]
...
To get the memory address of a variable we will use `&` character as follows.
...
The `int` type of the pointer tells the pointer what data type is stored in that memory address.
> The pointer's data type will helps us later on when we learn about dereferencing.

### Dereferencing
Dereference a pointer means getting the value of the memory address stored in the pointer. It can be done using `*` character which is also called dereference operator. 
...
```
### Why Pointers? 
Sometimes, when we want to update a value does not work as expected. Let us try to update a value using a function. 
